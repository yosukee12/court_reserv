...
class ReservationStatusWorkflowService:
    ...
    @staticmethod
    def save_remaining_accounts_csv(input_csv_path, cancelled_ids, output_csv_path):
        """Write the input CSV without IDs cancelled during this workflow."""
        cancelled = {str(user_id) for user_id in cancelled_ids}
        with open(input_csv_path, newline="", encoding="utf-8-sig") as source:
            rows = list(csv.reader(source))
        remaining = [row for row in rows if not row or row[0] not in cancelled]
